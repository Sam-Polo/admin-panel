...
Административная веб-панель для управления информацией объектов из Firestore. Разрабатывалась как дополнение к [мобильному приложению SportYugra](https://github.com/Sam-Polo/sportYugra)

<img src="https://github.com/user-attachments/assets/c8250e1f-6543-4c84-a552-075ac7959d7a" alt="Screenshot_210" width="90%"/>
...
```javascript
{
  user_id: string,       // id пользователя, выполнившего действие
  user_email: string,    // email пользователя
  action: string,        // тип действия (добавление, удаление, изменение и т.д.)
  details: object,       // детали операции в виде объекта
...
### Журнал действий (аудит-логи)
- Добавлена система логирования действий пользователей
- Реализован интерфейс для просмотра журнала (доступен только администраторам)
- Автоматическая запись всех важных действий пользователей:
...
- Расширенные фильтры для работы с журналом:
  - Поиск по текстовому содержимому
  - Фильтрация по типу действия
  - Фильтрация по диапазону дат
- Возможность просмотра подробной информации о каждом действии
- Структурированное отображение деталей операции в модальном окне
- Интерфейс для удаления устаревших записей журнала
...
- Улучшено модальное окно для просмотра деталей операций:
  - Структурированное отображение информации с разделением по строкам
  - Выделение ключевых полей для лучшей читаемости
  - Адаптивный дизайн для отображения различных типов данных
